...
//向IIC总线写数据
void OLED_Write_IIC_Byte(uint8_t _ucByte)
{
  uint8_t i;
...
{
	OLED_IIC_Start();
	OLED_Write_IIC_Byte(OLED_ADDRESS);//OLED地址
	OLED_IIC_WaitAck();
	OLED_Write_IIC_Byte(0x00);//寄存器地址
	OLED_IIC_WaitAck();
...
void OLED_Write_IIC_Data(u8 IIC_Data)
{
	...
	OLED_Write_IIC_Byte(OLED_ADDRESS);//OLED地址
	OLED_IIC_WaitAck();
	OLED_Write_IIC_Byte(0x40);//寄存器地址
	OLED_IIC_WaitAck();
...
{ 
	...
	OLED_Write_IIC_Command((((x+2)&0xf0)>>4)|0x10);
	OLED_Write_IIC_Command(((x+2)&0x0f)|0x01);
}
...
void OLED_ShowChar(u8 x, u8 y, u8 chr)
{
  ...
  if(x > 130 - 1)
  {
	...
	OLED_Set_Pos(x, y);
	for(i = 0; i < 8; i++)
		OLED_Write_IIC_Data(zf[c * 16 + i]);
	OLED_Set_Pos(x, y + 1);
	for(i = 0; i < 8; i++)
		OLED_Write_IIC_Data(zf[c * 16 + i + 8]);
	...
	while (*chs != '\0')
	{
		if (*chs > 0xa0)				//汉字内码都是大于0xa0
		{
			...
			{	
				...
				if ((*chs == hz_index[i*2]) && (*(chs+1) == hz_index[i*2+1]))
				{
					OLED_Set_Pos(x, y);
					for(j=0;j<16;j++)
						OLED_Write_IIC_Data(hz[i*32+j]);
					OLED_Set_Pos(x,y+1);
					for(j=0;j<16;j++)
						OLED_Write_IIC_Data(hz[i*32+j+16]);
					x +=16;
					break;
				}
			}
		...
		else
		{
			...
			if (m!=NULL)
			{
				OLED_Set_Pos(x, y);
				for(j = 0; j < 8; j++)
					OLED_Write_IIC_Data(zf[((u8)*m-' ') * 16 + j]);
				OLED_Set_Pos(x, y + 1);
				for(j = 0; j < 8; j++)
					OLED_Write_IIC_Data(zf[((u8)*m-' ') * 16 + j + 8]);
				x += 8;
			}
			chs++;
		}
...
void OLED_Init(void)
{
  ...
  OLED_Write_IIC_Command(0xB0); //--set page address
  ...
  OLED_Write_IIC_Command(0x3F); //--1/32 duty
  OLED_Write_IIC_Command(0xC8); //Com scan direction
  ...
  OLED_Write_IIC_Command(0xD8); //set area color mode off
  OLED_Write_IIC_Command(0x05); //
